...
namespace Disassembler
{
    public class Program
    {
        ...
        {
            var options = new Settings(int.Parse(args[0]), args[1], true, false, true, false, 0, "disassembly.html");
        ...
        static void Handle(Settings settings)
        {
                ...
                AttachFlag.Invasive))
            {
                ...
                using (var writer = new StreamWriter(settings.ResultsPath))
                    new DisassemblyExporter(new DisassemblyResult { Methods = disasembledMethods.ToArray() }, settings.TypeName).Export(writer);
            }
        }
        ...
        static List<DisassembledMethod> Disassemble(Settings settings, ClrRuntime runtime, State state)
        {
            ...
            foreach (var clrMethod in typeWithBenchmark.Methods.Where(m => m.IsPublic))
            {
                Console.WriteLine(": " + clrMethod.Name);
        ...
        static ClrMethod GetMethod(MethodReference methodReference, ClrType declaringType)
        {
            ...
            return declaringType
                .Methods
                .FirstOrDefault(method => method.GetFullSignature() == unifiedSignature);
        }
    ...
    {
        public Settings(int processId, string typeName, bool printAsm, bool printIL, bool printSource, bool printPrologAndEpilog, int recursiveDepth, string resultsPath)
        {
            ProcessId = processId;
            TypeName = typeName;
            PrintAsm = printAsm;
